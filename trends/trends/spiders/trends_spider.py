...
class TrendsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        trends_data = []
        print("Inside Parsing within the scrapy spider crawler.")
        print(f"Printing Trends Data before the css element scraping: {trends_data}")
        ...
        #     time_ago = trend.css("div.A7jE4::text").get()
        #     trends_data.append({"keyword": keyword, "related": related, "time_ago": time_ago})
        print(f"Printing Trends Data After the css element scraping: {trends_data}")
        self.shared_data.extend(trends_data)
        print(f"Printing Shared_Data after extending it whithin the crawler: {self.shared_data}")